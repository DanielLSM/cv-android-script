...
# Fixes the frame to designed rectangle. Default is 256-256
    def set_frame(self):
        cv2.rectangle(self.frame, (int((self.fw-self.rw)/2),int((self.fl-self.rl)/2)), (int((self.fw+self.rw)/2),int((self.fl+self.rl)/2)), (0,125,230), 3);
# Returns the center of the frame as a x,y tuple
    def set_center(self):
        cv2.circle(self.frame, (int(self.cap.get(3) / 2), int(self.cap.get(4) / 2)), 4, (0,125,230), 4, 5);
# Send the deep copy not the actualy frame. This can be modified safely
    ...
    def get_subframe(self):
        ...
        return copy.copy(self.frame[ int((self.fl-self.rl)/2):int((self.fl+self.rl)/2),int((self.fw-self.rw)/2):int((self.fw+self.rw)/2) ])
    def clear(self):
        self.cap.release()
